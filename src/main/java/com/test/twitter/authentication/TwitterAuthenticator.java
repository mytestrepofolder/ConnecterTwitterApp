package com.test.twitter.authentication;
...
public class TwitterAuthenticator {
    ...
    private static final Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("amsdc1-cc-30001.europe.shell.com", 8080)); //Added  for proxy support (Remove it when not needed)
    private static final HttpTransport TRANSPORT = new NetHttpTransport.Builder().setProxy(proxy).build();//Added by for proxy support (Remove it when not needed)
